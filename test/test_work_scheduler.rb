...
module UDSim
  class TestWorkScheduler < Test::Unit::TestCase
    ...
    def setup
      ...
      WorkScheduler.reset_gantt_data
    ...
    def test_dynamic_task_addition
      ...
      # UDSim::WorkScheduler.plot_gantt_chart("debug_dynamic_task_addition.html")
    ...
    def test_gantt_chart_generation
      ...
      chart_file = "/tmp/test_gantt_chart.html"
      UDSim::WorkScheduler.plot_gantt_chart(chart_file)
      # UDSim::WorkScheduler.plot_gantt_chart("debug_test_gantt_chart.html")

      # Verify file was created
      assert File.exist?(chart_file), "Gantt chart file should be created"

      # Read and verify file contents
      content = File.read(chart_file)
      assert content.include?("WorkScheduler Gantt Chart"), "Should contain chart title"
      assert content.include?("google.visualization.Timeline"), "Should use Google Charts Timeline"
      assert content.include?("Engineer1"), "Should contain engineer names"
      assert content.include?("Engineer2"), "Should contain engineer names"
      assert content.include?("Task1"), "Should contain task names"

      # Clean up
      File.delete(chart_file) if File.exist?(chart_file)
    end

    # Test 8: Edge case - all people busy
